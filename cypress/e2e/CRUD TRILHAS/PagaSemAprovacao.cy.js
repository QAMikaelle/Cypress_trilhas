...
const TRAIL_NAME = `Trilha Paga sem Aprovação ${RANDOM_ID}`;
...
describe(`Trilha Paga SEM aprovação do gestor (${TRAIL_NAME})`, () => {
    ...
    it("Criar trilha paga sem aprovação, clonar turma como gratuita e adicionar capas", () => {
        // ========== CRIAR TRILHA ==========
        ...
        cy.get('#currentClassPrice', { timeout: 60000 })
            ...
            .scrollIntoView().click({ force: true }).focus()
            .clear({ force: true })
            .type('79.90', { delay: 50, force: true })
            .blur();

        // SEM aprovação do gestor (não clicamos no requireApproval)
        cy.get('.editing-class > :nth-child(1) > .content-box-footer > .btn-swipe-accent').click();
        cy.wait(3000);

        // ========== CLONAR TURMA COMO GRATUITA ==========
        ...
        cy.log('✅ Verificando turmas clonadas: paga + gratuita');
        cy.get('body').then(($body) => {
    ...
    it("Aluno acessa e testa fluxo de compra sem aprovação", () => {
        trocarPerfil(perfilAluno);

        cy.log('⏸️ PAUSA: Navegue para a URL da trilha paga criada. Verifique se o botão "Efetuar pagamento" está disponível sem precisar de aprovação. (15s)');
        cy.wait(15000);

        cy.log('⏸️ PAUSA: Verifique se é possível comprar a trilha. Após a compra valide se o acesso ao treinamento é liberado. (15s)');
        cy.wait(15000);

        cy.log('⏸️ PAUSA: Conclua o treinamento e verifique se é possível acessar a trilha após conclusão. (15s)');
        cy.wait(15000);
    });
});
